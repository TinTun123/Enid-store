<template>
    <MainLayout>
        <div id="ItemPage" class="max-w-[1200px] mx-auto px-2 mt-8">
            <div class="md:flex gap-4 justify-between mx-auto w-full">
                <div class="md:w-[30%] aspect-square">
                    <img
                    v-if="currentImage"
                    :src="currentImage" class="rounded-lg object-fit" alt="">
                    <div v-if="images[0] !== ''" class="flex items-center justify-center mt-2">
                        <div v-for="image in images">
                            <img 
                            :src="image" 
                            @mouseover="currentImage = image" 
                            @click="currentImage = image"  width="70" 
                            class="rounded-md object-fit border-[3px] cursor-pointer"
                            :class="currentImage === image ? 'border-[#FF5353]' : '' "  alt="">
                        </div>
                    </div>
                </div>

                <div class="md:w-[60%] bg-white p-3 rounded-lg">
                    <div v-if="product && product.data">
                        <p class="mb-2">
                            {{ product.data.title }}
                        </p>
                        <p class="font-light text-[12px] mb-2">
                            {{ product.data.description }}
                        </p>
                    </div>

                    <div class="flex items-center pt-1.5">
                        <span class="h-4 min-w-4 rounded-full p-0.5 bg-[#FFD000] mr-2">
                            <Icon name="material-symbols:star-rounded" class="-mt-[17px]" size="12"/>
                        </span>
                        <p class="text-[#FF3535]">
                            Extra 5% off
                        </p>
                    </div>

                    <div class="flex items-center justify-start gap-x-1 my-2">
                        <Icon name="ic:baseline-star" color="#FF5353"/>
                        <Icon name="ic:baseline-star" color="#FF5353"/>
                        <Icon name="ic:baseline-star" color="#FF5353"/>
                        <Icon name="ic:baseline-star" color="#FF5353"/>
                        <Icon name="ic:baseline-star" color="#FF5353"/>
                        <span>
                            5 213 Reviews 1,000+ orders
                        </span> 
                    </div>

                    <div class="border-b">
                        <div class="flex font-bold items-center justify-start gap-2 my-2">
                            <div class="text-xl font-bold">
                                ${{ priceComputed }}

                            </div>
                            <span class="bg-[#5f5f5f] border text-[#C08562] text-[9px] font-semibold px-1.5 rounded-sm">
                                70% off
                            </span>
                        </div>

                        <p class="text-[#009A66] text-sm font-semibold pt-1">
                            Free 11 days delivery over $8.82
                        </p>

                        <div class="py-2">
                            <button :disabled="isInCart" @click="addToCart()" class="px-6 py-2 rounded-lg text-white text-lg font-semibold bg-gradient-to-t from-[#FF851A] to-[#FFAC2C]">
                                <div v-if="isInCart">
                                    Is Added
                                </div>
                                <div v-else>
                                    Add to Cart
                                </div>
                            </button>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </MainLayout>

</template>

<script setup>

import MainLayout from '~/layouts/MainLayout.vue';
import { useUserStore } from '../../stores/userStore';
import {useRoute} from 'vue-router';

const currentImage = ref(null);
const userStore = useUserStore();
const route = useRoute();
const product = ref(null);


onBeforeMount(async () => {
    product.value = await useFetch(`/api/prisma/get-product-by-id/${route.params.id}`);
})

onMounted(() => {
    console.log(userStore.cart);
})

watchEffect(() => {
    if (product.value && product.value.data) {

        currentImage.value = product.value.data.url;
        images.value[0] = product.value.data.url;
        userStore.isLoading = false;
    }
})

setTimeout(() => userStore.isLoading = false, 800);


const priceComputed = computed(() => {
    if (product.value && product.value.data){
        return (product.value.price / 100).toFixed(2);
    }

    return 0.00
})

const images = ref([
    '',
    'https://picsum.photos/id/212/800/800',
    'https://picsum.photos/id/233/800/800',
    'https://picsum.photos/id/165/800/800',
    'https://picsum.photos/id/99/800/800',
    'https://picsum.photos/id/144/800/800'
])

const addToCart = () => {
    
    userStore.cart.push(product.value.data);
}

const isInCart = computed(() => {
    let res = false;
    console.log(userStore.cart);
    userStore.cart.forEach(prod => {
        if (Number(route.params.id) === prod.id) {
            res = true;
        }
    })
    return res;
})

</script>

<style>
</style>